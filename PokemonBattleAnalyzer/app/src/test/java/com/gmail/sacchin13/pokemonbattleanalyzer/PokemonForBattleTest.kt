...
class PokemonForBattleTest {
    ...
    @Test
    fun 攻撃側の攻撃力計算のテスト() {
        val attackSide = PokemonForBattle.create(kucheat)
    ...
    fun 防御側の状態異常時の防御力計算のテスト() {
        val defenseSide = PokemonForBattle.create(kucheat)
    ...
    @Test
    fun ジャイロボールの威力のテスト() {
        var attackSide = PokemonForBattle.create(kucheat)
        attackSide.skill.jname = "ジャイロボール"
        var defenseSide = PokemonForBattle.create(bakuhun)

        var result = attackSide.determineSkillPower(defenseSide)
        assertEquals(43, result)


        attackSide = PokemonForBattle.create(tubotubo)
        attackSide.skill.jname = "ジャイロボール"
        defenseSide = PokemonForBattle.create(tekkanin)
    ...
    fun 固定2回攻撃技の威力のテスト() {
        ...
        val defenseSide = PokemonForBattle.create(bakuhun)
